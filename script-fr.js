...
function TextWidget() {
    this.serverURL = "", this.useEmailField = !0, this.usePhoneField = !0, this.useNameField = !0, this.useCustomField = !1, this.useMessageField = !1, this.customField = {}, this.locationId = "", this.heading = "", this.subHeading = "", this.headerIcon = "", this.footerText = "", this.buttonText = "", this.thankYouHeading = "", this.thankYouSubHeading = "", this.customIcon = "", this.countryCode = "US", this.primaryColor = "", this.formId = "", this.promptShow = !0, this.promptImage = "", this.promptMessage = "", this.nameField = function() {
        return ['              <div class="form-group">', '                <label class="sr-only">Nom</label>', '                <input type="text" class="form-control" placeholder="Nom" id="msgsndr_name" required value="' + this.getName() + '">', "              </div>"]
    }, this.phoneField = function() {
        return ['              <div class="form-group" id="phone-group">', '                <label class="sr-only">Téléphone Mobile</label>', '                <input type="text" class="form-control" placeholder="Téléphone Mobile" id="msgsndr_phone" required value="' + this.getPhone() + '">', "              </div>"]
    }, this.emailField = function() {
        return ['              <div class="form-group">', '                <label class="sr-only">Adresse Courriel</label>', '                <input type="email" class="form-control" placeholder="Adresse Courriel" id="msgsndr_email" required value="' + this.getEmail() + '">', "              </div>"]
...
TextWidget.prototype.getIcon = function() {
    ...
    return this.heading || "Bonjour ðŸ‘‹"
}, TextWidget.prototype.getSubHeading = function() {
    return this.subHeading || "Entrez votre question ci-dessous et nous vous répondrons sous peu."
}, TextWidget.prototype.getThankYouHeading = function() {
    return this.thankYouHeading || "Merci!"
}, TextWidget.prototype.getThankYouSubHeading = function() {
    return this.thankYouSubHeading || "Un de nos représentants vous contactera sous peu."
}, TextWidget.prototype.getFooterText = function() {
    return this.footerText || "En soumettant vous acceptez de recevoir des SMS ou courriels."
}, TextWidget.prototype.getButtonText = function() {
    return this.buttonText || "Envoyer"
}, TextWidget.prototype.getpromptMessage = function() {
    return this.promptMessage || "Bonjour, avez-vous une question? Posez-la ici."
    ...
    return this.promptShow ? '<div class="prompt-container" style="display: none"><div class="prompt-close" style="background:' + this.getPrimaryColor() + '">fermer</div><div class="prompt-msg" ><img class="prompt-profile" src="' + this.getPromptImage() + '" alt="prompt-profile"><div class="prompt-message">' + this.getpromptMessage() + "</div></div></div>" : ""
    ...
    console.log(e), msgsndrjQuery.ajax({
        ...
        error: function(e, t, o) {
            ...
            if (i && i.show(), e && e.responseText) {
                var s = JSON.parse(e.responseText),
                    n = msgsndrjQuery("#errmsgdisp");
                s && s.msg && n ? n.html(s.msg) : n && n.html("Veuillez saisir le nom, le numéro de téléphone ou l'adresse e-mail corrects et le message avant d'appuyer sur envoyer."), console.log(e.responseText)
            }
        }
    })
    ...
    return console.log("Valid number? " + o), o ? document.getElementById("msgsndr_phone").setCustomValidity("") : document.getElementById("msgsndr_phone").setCustomValidity("Veuillez fournir un numéro de téléphone valide"), this.updateRequiredAttribute("email", !1), t
    ...
    return console.log("Valid email? " + t), t ? document.getElementById("msgsndr_email").setCustomValidity("") : document.getElementById("msgsndr_email").setCustomValidity("Veuillez fournir une adresse courriel valide"), this.updateRequiredAttribute("phone", !1), e
